...
def execute_analysis_query(
    ...
    return _format_as_table(columns, rows)  # type: ignore


def _format_as_table(columns: list[str], rows: list[tuple[Any]]) -> str:  # type: ignore
    """Format results as an ASCII table."""
    # Convert all values to strings
    str_rows = [
        [str(val) if val is not None else "NULL" for val in row] for row in rows
    ]
    ...
    header = (
        "|"
        + "|".join(f" {col.ljust(widths[i])} " for i, col in enumerate(columns))
        + "|"
    )
    ...
    for row in str_rows:
        line = (
            "|"
            + "|".join(f" {val.ljust(widths[i])} " for i, val in enumerate(row))
            + "|"
        )
        lines.append(line)
