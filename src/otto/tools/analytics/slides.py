...
def generate_kpi_slide(dfs: Dict[str, pd.DataFrame], month: str) -> dict:
    ...
    """
    result = {"month": month, "kpis": [], "narrative": "", "key_metrics": {}}
    ...
    if not kpi_monthly_df.empty:
        ...
        if not metric_targets_df.empty:
            month_targets = metric_targets_df[
                metric_targets_df["fiscal_month"] == month
            ]
            month_kpis = month_kpis.merge(
        ...
        for kpi in kpi_records:
            ...
            if thresholds and target_value:
                try:
                    ...
                    variance_pct = (
                        ((kpi_value - target_value) / target_value) * 100
                        if target_value != 0
                        else 0
                    )
    ...
    if not commentary_library_df.empty:
        ...
        result["narrative"] = (
            ...
            if narrative_parts
            else "No commentary available for this period."
        )
    else:
        result["narrative"] = (
            f"Financial summary for {month}. Key metrics tracked across operational and financial dimensions."
        )
            ...
            for name, metric in list(result["key_metrics"].items())[:5]:  # Top 5
                ...
                if target:
                    key_metrics_list.append(
                        f"{name}: {value} (target: {target}, {status})"
                    )
                else:
