...
def calculate_runway(
    dfs: dict[str, pd.DataFrame], delay_capex_days: int = 0
) -> dict[str, Any]:
    """
    ...
    if cash_df.empty:
        return {
            ...
            "runway_months": 0.0,
            "projection": [],
            "summary": {"currency": "USD", "capex_delay_days": delay_capex_days},
    ...
    if not gl_df.empty and not dim_account_df.empty:
        ...
        opex_accounts = dim_account_df[dim_account_df["account_type"] == "Opex"][
            "account_id"
        ].tolist()
    ...
    if not capex_df.empty and delay_capex_days > 0:
        if "planned_month" in capex_df.columns:
            ...
            capex_df["planned_month_dt"] = pd.to_datetime(
                capex_df["planned_month"] + "-01"
            )
            capex_df["planned_month_dt"] = capex_df["planned_month_dt"] + timedelta(
                days=delay_capex_days
            )
            capex_df["planned_month"] = capex_df["planned_month_dt"].dt.strftime(
                "%Y-%m"
            )
    ...
    summary_parts.append(
        f"Current cash balance: ${current_cash:,.2f} (as of {latest_date.strftime('%Y-%m-%d')})."
    )
    summary_parts.append(
        f"Monthly burn rate: ${monthly_burn:,.2f} (based on trailing 3-month average)."
    )
    ...
    if runway_months == float("inf"):
        ...
        summary_parts.append(
            f"Cash runway: {runway_months:.1f} months ({years:.1f} years)."
        )
    else:
