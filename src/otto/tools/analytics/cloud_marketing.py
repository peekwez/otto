...
def cloud_marketing_breakdown(
    dfs: dict[str, pd.DataFrame], month: str
) -> dict[str, Any]:
    """
    ...
    if not cloud_df.empty:
        ...
        if not cloud_month.empty:
            ...
            # Group by provider
            cloud_by_provider = cloud_month.groupby("provider", as_index=False)[
                "amount"
            ].sum()
    ...
    if not marketing_df.empty:
        ...
        if not marketing_month.empty:
            ...
            # Group by channel
            marketing_by_channel_total = marketing_month.groupby(
    ...
    for item in result["cloud_costs"]:
        ...
        cloud_by_provider_dict[provider] = (
            cloud_by_provider_dict.get(provider, 0) + amount
        )
    ...
    for item in result["marketing_spend"]:
        ...
        amount = item.get("amount", 0)
        marketing_by_channel_dict[channel] = (
            marketing_by_channel_dict.get(channel, 0) + amount
        )

    top_provider = (
        max(cloud_by_provider_dict.items(), key=lambda x: x[1])
        if cloud_by_provider_dict
        else None
    )
    top_channel = (
        max(marketing_by_channel_dict.items(), key=lambda x: x[1])
        if marketing_by_channel_dict
        else None
    )
    ...
    if result["total_cloud"] > 0:
        summary_parts.append(
            f"Total cloud costs: ${result['total_cloud']:,.2f} across {cloud_count} service entries."
        )
        if top_provider:
            summary_parts.append(
                f"Top cloud provider: {top_provider[0]} with ${top_provider[1]:,.2f}."
            )
    else:
    ...
    if result["total_marketing"] > 0:
        summary_parts.append(
            f"Total marketing spend: ${result['total_marketing']:,.2f} across {marketing_count} campaign entries."
        )
        if top_channel:
            summary_parts.append(
                f"Top marketing channel: {top_channel[0]} with ${top_channel[1]:,.2f}."
            )
    else:
    ...
    if total_spend > 0:
        cloud_pct = (
            (result["total_cloud"] / total_spend * 100) if total_spend > 0 else 0
        )
        marketing_pct = (
            (result["total_marketing"] / total_spend * 100) if total_spend > 0 else 0
        )
        summary_parts.append(
    ...
    return {
        ...
        "summary": {
            ...
            "top_cloud_provider_amount": round(top_provider[1], 2)
            if top_provider
            else 0.0,
            "top_marketing_channel": top_channel[0] if top_channel else None,
            "top_marketing_channel_amount": round(top_channel[1], 2)
            if top_channel
            else 0.0,
            "currency": "USD",
        },
