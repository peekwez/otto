...
def burn_by_function(dfs: dict[str, pd.DataFrame]) -> dict[str, Any]:
    """
    ...
    burn_by_function = gl_df.groupby(["function", "fiscal_month"], as_index=False)[
        "amount_base"
    ].sum()

    # Get total by function across all months
    function_totals = burn_by_function.groupby("function", as_index=False)[
        "amount_base"
    ].sum()
    function_totals.rename(columns={"amount_base": "total_burn"}, inplace=True)

    # Get monthly average
    monthly_counts = burn_by_function.groupby("function", as_index=False)[
        "fiscal_month"
    ].count()
    ...
    function_summary["avg_monthly_burn"] = (
        function_summary["total_burn"] / function_summary["month_count"]
    )
    ...
    avg_monthly_burn_all = (
        sum(f.get("avg_monthly_burn", 0) for f in functions) / len(functions)
        if functions
        else 0.0
    )
    ...
    summary_parts.append(
        f"Average monthly burn per function: ${avg_monthly_burn_all:,.2f}."
    )
