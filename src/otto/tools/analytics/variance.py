...
def variance_report(
    dfs: dict[str, pd.DataFrame], fiscal_quarter: str, budget_version: str
) -> dict[str, Any]:
    """
    ...
    if month_to_quarter:
        ...
        return {
            ...
            "summary_text": "Could not map months to quarters. No variance report generated.",
        }

    # Aggregate actuals by dept_id and account_id
    actuals_agg = actuals_df.groupby(["dept_id", "account_id"], as_index=False)[
        "amount_base"
    ].sum()
    actuals_agg.rename(columns={"amount_base": "actual"}, inplace=True)

    # Aggregate budget by dept_id and account_id
    budget_agg = budget_df.groupby(["dept_id", "account_id"], as_index=False)[
        "amount_base"
    ].sum()
    budget_agg.rename(columns={"amount_base": "budget"}, inplace=True)

    # Merge actuals and budget
    variance_df = actuals_agg.merge(
        budget_agg, on=["dept_id", "account_id"], how="outer"
    )
    ...
    variance_df["variance_pct"] = (
        variance_df["variance"] / variance_df["budget"].replace(0, 1)
    ) * 100
    variance_df["variance_pct"] = variance_df["variance_pct"].replace(
    ...
    largest_unfavorable = next(
        (r for r in rows_sorted if r.get("variance", 0) > 0), None
    )

    # Generate human-readable summary
    summary_parts = []
    summary_parts.append(
        f"Variance report for {fiscal_quarter} using budget version {budget_version}."
    )
    summary_parts.append(
        f"Total actual: ${total_actual:,.2f}, Total budget: ${total_budget:,.2f}."
    )
    summary_parts.append(
        f"Total variance: ${total_variance:,.2f} ({total_variance_pct:+.1f}%)."
    )
